...
app.get("/diner-post/:id", (req, res, next) => {
    ...
    axios
        .get(url)
        .then((apiResponse) => {
            const postString = JSON.stringify(apiResponse.data)
                .replace(/["\\n]/g, "")
                .split(",");
            const post = {
                id: postString[0],
        ...
        .catch((err) => {
            // if mockaroo doesn't work, serve static sample data
            console.log(err);
            res.json({
...
app.get("/profile", function (req, res) {
    ...
    axios
        .get(url)
        .then((apiResponse) => {
            const userString = JSON.stringify(apiResponse.data)
                .replace(/["\\n]/g, "")
                .split(",");
            const user = {
                id: userString[0],
...
app.post("/create-post/:placeId", async (req, res) => {
    try {
        ...
        axios.get(url).then(async (apiResponse) => {
            const restaurant_data = apiResponse.data.result;
